...
if [ -e ./efaecium.ms16.histogram -a -e ./efaecium.ms16.dump -a -e ./efaecium.ms16.ignore.gz ] ; then
  exit 0
fi
...
if [ -e efaecium.ms16.histogram ]; then
  exists1=true
else
  exists1=false
fi

if [ -e efaecium.ms16.dump ]; then
  exists2=true
else
  exists2=false
fi

if [ -e efaecium.ms16.ignore.gz ]; then
  exists3=true
else
  exists3=false
fi
if [ $exists1 = true -a $exists2 = true -a $exists3 = true ] ; then
  echo "Output files 'efaecium.ms16.histogram', 'efaecium.ms16.dump' and 'efaecium.ms16.ignore.gz' exist in 'correction/0-mercounts'."
  exit 0
fi
...
if [ ! -e ./efaecium.ms16/merylIndex ] ; then
  /home/julievm/miniconda3/envs/ngs_efaecium/bin/meryl threads=4 memory=2 \
    greater-than 1 \
      output efaecium.ms16.WORKING \
      union-sum  \
        ./efaecium.01.meryl \
  && \
  mv -f ./efaecium.ms16.WORKING ./efaecium.ms16
  ...
  rm -rf ./efaecium.01.meryl ./efaecium.01.meryl.err
fi
...
if [ ! -e ./efaecium.ms16.histogram ] ; then
  /home/julievm/miniconda3/envs/ngs_efaecium/bin/meryl threads=1 memory=1 \
    statistics ./efaecium.ms16 \
  > ./efaecium.ms16.histogram.WORKING \
  && \
  mv ./efaecium.ms16.histogram.WORKING ./efaecium.ms16.histogram
fi
...
if [ ! -e ./efaecium.ms16.dump ] ; then
  /home/julievm/miniconda3/envs/ngs_efaecium/bin/meryl threads=4 memory=2 \
    print ./efaecium.ms16.##.dump \
      at-least threshold=112 \
      at-least word-frequency=0.0000001 \
        ./efaecium.ms16

  cat ./efaecium.ms16.??.dump > ./efaecium.ms16.dump
  rm -f ./efaecium.ms16.??.dump
fi
...
if [ ! -e ./efaecium.ms16.ignore.gz ] ; then
  numKmers=`wc -l < ./efaecium.ms16.dump`
  totKmers=`/home/julievm/miniconda3/envs/ngs_efaecium/bin/meryl statistics ./efaecium.ms16 | grep present | awk '{ print $2 }'`


  ./meryl-make-ignore.pl $numKmers $totKmers < ./efaecium.ms16.dump | gzip -1c > ./efaecium.ms16.ignore.gz
fi
