...
if [ -e ./efaecium.$jobid.meryl/merylIndex ] ; then
...
if [ -e efaecium.$jobid.meryl.tar.gz ]; then
  exist1=true
else
  exist1=false
fi
...
/home/julievm/miniconda3/envs/ngs_efaecium/bin/meryl k=16 threads=4 memory=2 \
  count \
    segment=$jobid/01 ../../efaecium.seqStore \
    output ./efaecium.$jobid.meryl.WORKING \
&& \
mv -f ./efaecium.$jobid.meryl.WORKING ./efaecium.$jobid.meryl
